...
const ContentEmail = ({ userEmail, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await adminWebsiteService.sendEmail(
        user._id,  // Admin ID
        userEmail, // Corrected: Using actual email
        subject,
        message
      );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-[99999] flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="p-4">
          {/* Updated label from 'To User ID:' to 'To (Email):' */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700">To (Email):</label>
            <input
              type="email"
              value={userEmail} // Corrected Prop Usage
              readOnly
